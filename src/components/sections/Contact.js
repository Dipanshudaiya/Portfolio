...
export default function Contact() {
    ...
    return (
        <section id="contact" className="bg-transparent py-[80px] md:py-[100px] w-full overflow-hidden">
            <AnimatePresence>
                {formStatus === 'sending' && (
                    <PageLoader isVisible={true} projectName="Sending Message..." onComplete={() => {}} />
                )}
            </AnimatePresence>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.8fr] gap-8 md:gap-12 items-start">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-1 gap-4 md:gap-6">
                        {[
                            ...
                            <div 
                                key={item.label} 
                                className={`relative overflow-hidden flex flex-col md:flex-row items-center md:items-center gap-4 md:gap-6 p-5 md:p-6 rounded-[2rem] md:rounded-[2.5rem] bg-white dark:!bg-[#0a0a0a] border border-white dark:border-white/10 shadow-xl transition-all duration-200 group hover:-translate-y-1 active:scale-[0.98] cursor-pointer after:absolute after:inset-0 after:bg-[radial-gradient(circle_at_top_left,_rgba(13,148,136,0.15),_transparent_60%)] after:opacity-0 hover:after:opacity-100 after:transition-opacity after:duration-500 after:pointer-events-none ${item.fullWidth ? 'col-span-2 lg:col-span-1' : 'col-span-1'}`}
                            >
                    ...
                    {/* Contact Form */}
                    <div className="relative bg-white dark:!bg-[#0a0a0a] hover:shadow-[0_0_50px_rgba(13,148,136,0.15)] hover:border-teal-300 dark:hover:border-teal-500/30 transition-all duration-200 rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-14 shadow-[0_20px_80px_rgba(0,0,0,0.08)] dark:shadow-[0_40px_100px_rgba(0,0,0,0.3)] border border-gray-100 dark:border-white/5 overflow-hidden">
                            ...
                            {formStatus === 'success' ? (
                                ...
                                <form className="relative z-10 space-y-6 md:space-y-10" onSubmit={handleSubmit}>
                                    ...
                                    <div className="pt-4 md:pt-6">
                                        <button
                                            type="submit"
                                            className="w-full py-5 md:py-6 rounded-2xl bg-teal-600 hover:bg-teal-700 text-white font-black text-xs md:text-sm uppercase tracking-[0.4em] shadow-[0_20px_40px_rgba(13,148,136,0.3)] transition-all duration-200 hover:scale-[1.01] active:scale-95 hover:-translate-y-1 flex items-center justify-center gap-4 group/btn"
                                        >
                                            SEND MESSAGE 
